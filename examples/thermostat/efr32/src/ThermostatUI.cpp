...
void ThermostatUI::DrawUI(GLIB_Context_t * glibContext)
{
    ...
    DrawCurrentTemp(glibContext, 72, 41, false);
    DrawFooter(glibContext, false, false);
...
void ThermostatUI::DrawFooter(GLIB_Context_t * glibContext, bool autoMode, bool heating)
{
    if (autoMode)
    {
        GLIB_drawStringOnLine(glibContext, "Mode : Auto", 10, GLIB_ALIGN_LEFT, 0, 0, true);
    }
    else
    {
        GLIB_drawStringOnLine(glibContext, "Mode : Man", 10, GLIB_ALIGN_LEFT, 0, 0, true);
    }

    if (heating)
    {
        GLIB_drawBitmap(glibContext, HEATING_COOLING_X, HEATING_COOLING_Y, HEATING_WIDTH, HEATING_HEIGHT, heating_bits);
    }
    else
    {
 ...
 */
void ThermostatUI::DrawCurrentTemp(GLIB_Context_t * glibContext, int8_t temp, int8_t setPoint, bool isCelsius)
{
    uint8_t tempArray[2];
    ...
    if (temp > 99 || temp < -99 || setPoint > 99 || setPoint < -99)
    {
    ...
    // Print SetPoint
    GLIB_drawStringOnLine(glibContext, setPointLine, 7, GLIB_ALIGN_LEFT, 67, 0, true);
}
