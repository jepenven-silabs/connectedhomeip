...
CHIP_ERROR AppTask::InitGroupSwitch()
{
    ...
    // Set ACL for Groups binding
    chip::Access::AccessControl::Entry entry;
    ReturnErrorOnFailure(chip::Access::GetAccessControl().PrepareEntry(entry));
    entry.SetFabricIndex(1);
    ReturnErrorOnFailure(entry.SetPrivilege(chip::Access::Privilege::kManage));
    ReturnErrorOnFailure(entry.SetAuthMode(chip::Access::AuthMode::kGroup));
    // ReturnErrorOnFailure(entry.AddSubject(nullptr, 1));
    ReturnErrorOnFailure(chip::Access::GetAccessControl().CreateEntry(nullptr, entry));
