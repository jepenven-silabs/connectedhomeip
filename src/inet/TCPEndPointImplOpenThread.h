 ...
 *      on Linux and BSD-derived systems) or OpenThread TCP protocol
 *      control blocks, as the system is configured accordingly.
 */

/**
 * This file declares an implementation of Inet::TCPEndPoint using OpenThread.
 */
...
class TCPEndPointImplOpenThread : public TCPEndPoint, public EndPointStateOpenThread
{
public:
    TCPEndPointImplOpenThread(EndPointManager<TCPEndPoint> & endPointManager) :
...
private:
    ...
    tcp_pcb * mTCP;          // OpenThread Transmission control protocol (TCP) control block.
    ...
    static err_t OpenThreadHandleConnectComplete(void * arg, struct tcp_pcb * tpcb, err_t OpenThreadErr);
    static err_t OpenThreadHandleIncomingConnection(void * arg, struct tcp_pcb * tcpConPCB, err_t OpenThreadErr);
    static err_t OpenThreadHandleDataReceived(void * arg, struct tcp_pcb * tpcb, struct pbuf * p, err_t err);
    static err_t OpenThreadHandleDataSent(void * arg, struct tcp_pcb * tpcb, u16_t len);
    static void OpenThreadHandleError(void * arg, err_t err);
};

using TCPEndPointImpl = TCPEndPointImplOpenThread;
